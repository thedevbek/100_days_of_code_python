...
def is_leap(year):
    """Determines if the provided year is a leap year or not.

    Args:
        year (int): year to check

    Returns:
        bool: True if year is leap year, False if not.
    """
    if year % 4 == 0:
        if year % 100 == 0:
            if year % 400 == 0:
...
def days_in_month(year, month):
    """Returns the number of days of the month in the year provided

    Args:
        year (int): Year to use
        month (int): Month to use
